...
// Helper: normaliza un campo que puede ser DATETIME nativo O STRING en formato
// DD/MM/YYYY HH:MM:SS (fuente legada) o YYYY-MM-DD HH:MM:SS[.ms] (fuente nueva).
// Siempre devuelve DATETIME o NULL.
const toTS = (field) =>
  `COALESCE(
    SAFE.PARSE_DATETIME('%Y-%m-%dT%H:%M:%E*S', CAST(${field} AS STRING)),
...
export async function getPedidos({ desde, hasta, extraWhere = "" } = {}) {
  const sql = `
    SELECT
      ...
      ROUND(DATETIME_DIFF(${toTS('estado_asignando')},      ${TS_CREACION},                   MINUTE), 1) AS min_prep,
      -- Etapa 2: Demora en asignación (asignando → P_TS)
      ROUND(DATETIME_DIFF(${P_TS},                          ${toTS('estado_asignando')},      MINUTE), 1) AS min_asignacion,
      -- Etapa 3: Demora en llegar a tienda (camino_tienda → recibiendo)
      ROUND(DATETIME_DIFF(${toTS('estado_recibiendo')},     ${toTS('estado_camino_tienda')},  MINUTE), 1) AS min_viaje,
      -- Etapa 4: Demora en entrega al driver (recibiendo → camino_entrega)
      ROUND(DATETIME_DIFF(${toTS('estado_camino_entrega')}, ${toTS('estado_recibiendo')},     MINUTE), 1) AS min_pickup,
      -- Etapa 5: Demora en entrega cliente (entregando → finalizado)
      ROUND(DATETIME_DIFF(${toTS('estado_finalizado')},     ${toTS('estado_entregando')},     MINUTE), 1) AS min_reparto,
      ...
      -- Timestamps de cada estado (normalizados a YYYY-MM-DD HH:MM:SS)
...
export async function getLocales({ desde, hasta, extra