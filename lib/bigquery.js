...
// --- Umbrales -----------------------------------------------------------------
const U_PREP   = 25;   // 1. Tienda: asignando − creación
const U_ASIG   = 5;    // 2. Asignación: P_TS − asignando
const U_VIAJE  = 10;   // 3. Viaje a tienda: recibiendo − camino_tienda
const U_PICKUP = 5;    // 4. Entrega al driver: camino_entrega − recibiendo
const U_REP    = 12;   // 5. Entrega cliente: finalizado − entregando
const U_OBJ    = 45;   // Objetivo total
const U_MULTI  = 45;   // Objetivo multipedido

// P_TS: momento en que el driver confirma / inicia ruta al comercio
// estado_aceptado primero; si es null (Yango) usa estado_camino_tienda
const P_TS = `COALESCE(estado_aceptado, estado_camino_tienda)`;

// --- Cadena de métricas por etapa ---------------------------------------------
// 1. Demora en tienda: creación → asignando (sistema empieza a buscar driver)
const MIN_PREP   = `DATETIME_DIFF(estado_asignando,      ${TS_CREACION},         MINUTE)`;
// 2. Demora en asignación: asignando → P_TS (driver aceptó / inició ruta)
//    2.1 Falta driver (cruce con programación) — datos pendientes de nueva fuente
//    2.2 Disponibilidad estimada = (finalizado − recibiendo) + 5 min — calculado en getPedidos
const MIN_ASIG   = `DATETIME_DIFF(${P_TS},               estado_asignando,       MINUTE)`;
// 3. Demora en llegar a tienda: camino_tienda → recibiendo
const MIN_VIAJE  = `DATETIME_DIFF(estado_recibiendo,     estado_camino_tienda,   MINUTE)`;
// 4. Demora en entrega de pedido al driver: recibiendo → camino_entrega
const MIN_PICKUP = `DATETIME_DIFF(estado_camino_entrega, estado_recibiendo,      MINUTE)`;
// 5. Demora en entrega: entregando → finalizado
const MIN_REP    = `DATETIME_DIFF(estado_finalizado,     estado_entregando,      MINUTE)`;
...
export async function getKPIs({ desde, hasta, extraWhere = "" } = {}) {
  const sql = `
    ...
    SELECT
      ...
      -- Causas (pedidos fuera de objetivo)
...
export async function getPedidos({ desde, hasta, extraWhere = "" } = {}) {
  const sql = `
    SELECT
      ...
      -- Etapa 1: Demora en tienda (creación → asignando)
      ROUND(DATETIME_DIFF(estado_asignando,      ${TS_CREACION},        MINUTE), 1)                  AS min_prep,
      -- Etapa 2: Demora en asignación (asignando → P_TS)
      ROUND(DATETIME_DIFF(${P_TS},               estado_asignando,      MINUTE), 1)                  AS min_asignacion,
      -- Etapa 3: Demora en llegar a tienda (camino_tienda → recibiendo)
      ROUND(DATETIME_DIFF(estado_recibiendo,     estado_camino_tienda,  MINUTE), 1)                  AS min_viaje,
      -- Etapa 4: Demora en entrega al driver (recibiendo → camino_entrega)
      ROUND(DATETIME_DIFF(estado_camino_entrega, estado_recibiendo,     MINUTE), 1)                  AS min_pickup,
      -- Etapa 5: Demora en entrega cliente (entregando → finalizado)
      ROUND(DATETIME_DIFF(estado_finalizado,     estado_entregando,     MINUTE), 1)                  AS min_reparto,
      -- Retorno estimado: desde recojo en tienda hasta finalizado + 5 min buffer
      ROUND(DATETIME_DIFF(estado_finalizado,     estado_recibiendo,     MINUTE) + 5, 1)              AS min_retorno_est,
      -- Timestamps de cada estado
      CAST(estado_asignando    AS STRING)                                                             AS ts_asignando,
      CAST(${P_TS}             AS STRING)                                                             AS ts_pickup,
      CAST(estado_camino_tienda AS STRING)                                                            AS ts_camino_tienda,
      CAST(estado_recibiendo   AS STRING)                                                             AS ts_recibiendo,
      CAST(estado_camino_entrega AS STRING)                                                           AS ts_camino_entrega,
      CAST(estado_entregando   AS STRING)                                                             AS ts_entregando,
      CAST(estado_finalizado   AS STRING)                                                             AS ts_finalizado,
      -- Hora estimada de nueva disponibilidad del driver (finalizado + 5 min)
      CAST(DATETIME_ADD(estado_finalizado, INTERVAL 5 MINUTE) AS STRING)                             AS ts_disponible
    FROM ${VIEW()}
    WHERE ${fechaWhere(desde, hasta)}
      AND ${FILTRO_BASE}
      ${extraWhere}
...
// --- Volumen y cumplimiento por hora del dia ---------------------------------
