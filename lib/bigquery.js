...
// ─── KPIs por polígono ────────────────────────────────────────────────────────
export async function getKPIsPorPoligono({ desde, hasta } = {}) {
  const sql = `
    WITH base AS (
      SELECT *,
        ${MIN_ENTREGA} AS min_entrega,
...
export async function getKPIsPorHora({ desde, hasta } = {}) {
  const sql = `
    WITH base AS (
      SELECT *,
        ${MIN_ENTREGA} AS min_entrega,
...
export async function getTopConductores({ desde, hasta } = {}) {
  const sql = `
    WITH base AS (
      SELECT *,
        ${MIN_ENTREGA} AS min_entrega,
    ...
    SELECT
      ...
      COUNT(*)                                                           AS total,
      COUNTIF(min_entrega <= objetivo_min)                               AS dentro_obj,
      ROUND(COUNTIF(min_entrega <= objetivo_min) / COUNT(*) * 100, 1)   AS pct_ok,
      ROUND(AVG(min_entrega), 1)                                         AS avg_min
    FROM base
