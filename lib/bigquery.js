...
const FILTRO_BASE = `estado_entregado IS NOT NULL AND fecha_creacion IS NOT NULL AND hora_creacion IS NOT NULL
  AND TRIM(local) NOT IN ('DT San Borja', 'DT San Miguel', 'DT Surco')`;
...
// ─── Tendencia diaria de TODAS las etapas ────────────────────────────────────
export async function getTendenciaEtapas({ desde, hasta, extraWhere = "" } = {}) {
  const sql = `
    ${LOCAL_POL_CTE},
    base AS (
      SELECT
    ...
    SELECT
      ...
      ROUND(COUNTIF(min_entrega <= ${U_OBJ}) * 100.0 / COUNT(*), 1)             AS pct_ok,
      ROUND(AVG(min_prep),   1) AS avg_prep,
      ROUND(COUNTIF(min_prep   > ${U_PREP})   * 100.0 / COUNT(*), 1)            AS pct_prep,
      ROUND(AVG(min_asig),   1) AS avg_asig,
      ROUND(COUNTIF(min_asig   > ${U_ASIG})   * 100.0 / COUNT(*), 1)            AS pct_asig,
      ROUND(AVG(min_viaje),  1) AS avg_viaje,
      ROUND(COUNTIF(min_viaje  > ${U_VIAJE})  * 100.0 / COUNT(*), 1)            AS pct_viaje,
      ROUND(AVG(min_pickup), 1) AS avg_pickup,
      ROUND(COUNTIF(min_pickup > ${U_PICKUP}) * 100.0 / COUNT(*), 1)            AS pct_pickup,
      ROUND(AVG(min_rep),    1) AS avg_rep,
      ROUND(COUNTIF(min_rep    > ${U_REP})    * 100.0 / COUNT(*), 1)            AS pct_rep
    FROM base
    GROUP BY fecha
    ORDER BY fecha
  `;
...
// ─── Todas las etapas por hora ────────────────────────────────────────────────
export async function getEtapasPorHora({ desde, hasta, extraWhere = "" } = {}) {
  const sql = `
    ${LOCAL_POL_CTE}
    SELECT
      CAST(SPLIT(t.hora_creacion, ':')[OFFSET(0)] AS INT64) AS hora,
...
// ─── Todas las etapas por driver ──────────────────────────────────────────────
export async function getDriverEtapas({ desde, hasta, extraWhere = "" } = {}) {
  const sql = `
    ${LOCAL_POL_CTE}
    SELECT
      t.nombre_conductor,
      COUNT(*) AS total,
      ROUND(AVG(${MIN_ENTREGA}), 1) AS avg_entrega,
      ROUND(COUNTIF(${MIN_ENTREGA} <= ${U_OBJ}) * 100.0 / COUNT(*), 1) AS pct_ok,
      ROUND(AVG(${MIN_PREP}),    1) AS avg_prep,
      ROUND(COUNTIF(${MIN_PREP}   > ${U_PREP})   * 100.0 / COUNT(*), 1) AS pct_prep,
      ROUND(AVG(${MIN_ASIG}),    1) AS avg_asig,
...
// ─── KPIs por local (tienda) ──────────────────────────────────────────────────
export async function getKPIsPorLocal({ desde, hasta, extraWhere = "" } = {}) {
  const sql = `
    ${LOCAL_POL_CTE},
    base AS (
      SELECT
        t.local,
        COALESCE(lp.poligono, 'Sin polígono') AS poligono,
        ${MIN_ENTREGA} AS min_entrega,
        ${MIN_PREP}    AS min_prep,
        ${MIN_ASIG}    AS min_asig,
        ${MIN_VIAJE}   AS min_viaje,
...
export async function getSchema() {
  const sql = `
    ...
    WHERE table_name = '${process.env.BQ_TABLE || "report_order_log